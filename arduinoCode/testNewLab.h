...
class laby {
    ...
    public:
        ...
        /**function used to rotate a piece of the labyrinth
        */
        void rotateMatrix(int pos) {
            uint8_t leftPart=lab[pos/2]>>4;//extrait la partie de gauche de l'octet
            uint8_t rightPart= lab[pos/2] & 0xF;//extrait la partie de droite de l'octet
            if (pos%2==0) {
                leftPart = ((leftPart << 3) | (leftPart >> 1) ) & 0xF; //effectue un shift de bits circulaire, shift les bits a gauche et y ajouter le bit qui manque au debut
            }else {
                rightPart = ((rightPart << 3) | (rightPart >> 1) ) & 0xF;
            }
            lab[pos/2]= leftPart << 4 | rightPart;//recolle les deux parties
        }
        ...
        */
        ...
        /* add a  direction /put at 1 a specific bit
        */
        ...
        /*clear a bit at x potion used in the algo to create the lab
        */
        ...
        */
        void shuffleMatrix(){
            for(int i=1; i <63 ; i++){
                ...
                for(int y=1; y <randomNum ; y++){
                    rotateMatrix(i);
                }
            }
        }

        /*create the lab, algo by this vid https://www.youtube.com/watch?v=zbXKcDVV4G0
        */
        ...
        */
        void objectivesUpdate(){
            /* if (posPlayer == objectifsPos[0] and !objectifsBool[0]){
                objectifsBool[0]=true;
            }
            else if (posPlayer == objectifsPos[1] and !objectifsBool[1]){
                objectifsBool[1]=true;
            }*/
            fin = posPlayer==(TAILLE*TAILLE)-1;
        }
};
