...
class labV2_3D {
  ...
  const Line fw1[6] = {  // the vertical link between the wall on the left and the wall on the middle or the way to the front door
    { 4, 4, 4, 59 },
    { 1, 1, 1, 62 },
    { 16, 16, 16, 47 },
    { 13, 13, 13, 50 },
  ...
  const Line fw2[5] = {  // the vertical link between the wall on the left and the door on the left, appears in the door in the animation
    { 16, 16, 16, 47 },
    { 11, 11, 11, 52 },
    { 7, 7, 7, 56 },
    { 3, 3, 3, 60 },
  ...
  const Line fw3[6] = {  // wall on the left after the door, this part is mandatory
    { 18, 18, 18, 18 },
    { 14, 14, 16, 16 },
    { 10, 10, 11, 11 },
    { 4, 4, 7, 7 },
  ...
  const Line fw5[6] = {  // wall that can be seen from the front and that goes to  the left door
    { 0, 4, 4, 4 },
    { 0, 1, 1, 1 },
    { 12, 16, 16, 16 },
    { 8, 13, 13, 13 },
  ...
  const Line fw7[4] = {  //the next front door
    { 16, 16, 18, 18 },
    { 13, 13, 18, 18 },
    { 10, 10, 19, 19 },
    { 8, 8, 19, 19 }
  };
  const Line fw8[7] = {  //the left wall if no door
    { 0, 0, 4, 4 },
    { 0, 0, 1, 1 },
    { 16, 16, 18, 18 },
    { 11, 11, 16, 16 },
  ...
  const Line fw9[4] = {  // the next front door if no wall
    { 16, 16, 47, 16 },
    { 13, 13, 50, 13 },
    { 10, 10, 53, 10 },
    { 8, 8, 55, 8 }
  };
  const Line fw10[3] = {  //the next front door if there is a door but it's blocked
    { 20, 20, 31, 20 },
    { 20, 20, 20, 43 },
    { 20, 43, 31, 43 }
  };
  const Line fw11[5][5] = {  // te
    { { 25, 28, 31, 22 },
      { 25, 28, 31, 34 },
      { 25, 28, 25, 40 },
      { 25, 40, 31, 46 },
  ...
  const Line middleDoorClosed[12][8] = {
    { { 8, 8, 17, 17 },  //TODO : reduire les trait en effectuant seulement la partie haute
      { 18, 18, 45, 18 },
      { 45, 18, 45, 45 },
      { 46, 17, 55, 8 },
      { 46, 46, 55, 55 },
...
public:
  ...
  bool getDC[2]={true,true};

  /*calculate the lines from two points and reverse them verticaly /+ horizontaly if some parameter are true
  */
  void showPoint(Line l, uint64_t t[64], bool mirrorHor, bool mirrorVert) {
    ...
    if (a != 0) {
      if (!mirrorVert) {
        ...
        l = { int(63 - l.x1), l.y1, int(63 - l.x2), l.y2 };
        a = (l.x1 - l.x2);
        a = (l.y1 - l.y2) / a;
        int b = l.y1 - (l.x1 * a);  //TODO : si ça casse c'est ça
  ...
  /**
 reverse order of bits for the display packs ot bits
 */
  ...
  /**
create the matrix of bytes used by the protocol i2c -> bytes where each bit is a boolean that will tell us if you should light the pixel
this function is only for the animation used to rotate the room of the labyrinth
*/
  void affichageAnim(uint8_t frame, byte packets[512], bool frontDoorClosed, bool rightDoorClosed) {  //TODO : créer une liste de référencement
    ...
    if (options[1]) {                  //affichage d'en face su il y a une porte
      ...
      if (!frontDoorClosed) {                //si la porte en face est ouverte
        for (uint8_t i = 0; i < 12; ++i) {  // porte du centre rotation vers la gauche
        ...
        for (uint8_t i = 0; i < 8; ++i) {  // porte du centre rotation vers la gauche
          if (!(middleDoorClosed[frame][i].x1 == 0 and middleDoorClosed[frame][i].x2 == 0 and middleDoorClosed[frame][i].y1 == 0 and middleDoorClosed[frame][i].y2 == 0)) {
            // les animations peuvent etre entre 2 et 12 droites, j'arrete les animations des que je croise un point allant de (0,0) a (0,0)
            showPoint(middleDoorClosed[frame][i], tabAffichage, false, false);
          }
        }
      }
    ...
    if (options[2]) {  //affichage de la partie droite si il y a une porte
      ...
      if (frame > 0) {          // affichage de la middle door inversé por montrer la nouvelle porte
        if (!rightDoorClosed) {  //si la porte est ouverte
        ...
        }else {  // dans le cas ou le passage vers la porte existe mais elle est fermé
          for (uint8_t i = 0; i < 8; ++i) {
            if (!(middleDoorClosed[12 - frame][i].x1 == 0 and middleDoorClosed[12 - frame][i].x2 == 0 and middleDoorClosed[12 - frame][i].y1 == 0 and middleDoorClosed[12 - frame][i].y2 == 0)) {
              showPoint(middleDoorClosed[12 - frame][i], tabAffichage, false, true);
            }  //TODO : mettre  un break ou un while
          }
        }
    ...
    /**
create the matrix of bytes used by the protocol i2c -> bytes where each bit is a boolean that will tell us if you should light the pixel
this function is only for the animation used to move forward in the next room
*/
  void afficheForward(uint8_t frame, bool oldOptions[4], byte packets[512],bool object) {
    uint64_t tabAffichage[64] = {};
    ...
    if (frame > 2) {  //the little wall part juste at the angles of the door, mandatory part
      ...
      if(object){
        for (uint8_t i = 0; i < 5; i++) {
          showPoint(fw11[frame - 4][i], tabAffichage, false, true);
          showPoint(fw11[frame - 4][i], tabAffichage, false, false);
        }
      }
    }
    for (int i = 0; i < 512; i++) {
