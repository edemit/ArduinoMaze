...
class labV2_3D {
  ...
  const Line middleDoorwayNoDoor[12][8] = {
    { { 8, 8, 17, 17 },
      { 18, 18, 45, 18 },
      { 45, 18, 45, 45 },
      { 46, 17, 55, 8 },
...
*/
  void affichageAnim(uint8_t frame, byte packets[512]) {  //TODO : créer une liste de référencement
...
*/
  void afficheForward(uint8_t frame, bool oldOptions[4], byte packets[512], bool doorOpen) {
    uint64_t tabAffichage[64] = {};
    ...
    if(frame > 7){
      ...
      if (doorOpen){
        for (uint8_t i = 0; i < 4; i++) {
          showPoint(fw9[i], tabAffichage, false, true);
          showPoint(fw9[i], tabAffichage, false, false);
        }
      }else{
        for (uint8_t i = 0; i < 3; i++) {
          showPoint(fw10[i], tabAffichage, false, true);
          showPoint(fw10[i], tabAffichage, false, false);
        }
      }
    }
